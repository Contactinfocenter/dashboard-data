        ...
        function getCallerId(item, docKey) {
            return String(item?.Client_ID || item?.phone_number || item?.email || docKey);
        }
        ...
        function render() {
            ...
            const grandUniqueTotals = lastDates.map((_, i) => Object.values(perRegion).reduce((sum, r) => sum + r.dailyUniques[i], 0));
        ...
        function renderZoneSection() {
            ...
            function buildZoneTableHtml() {
                ...
                const grandUniques = lastDates.map((_, i) => zonesOfInterest.reduce((s, z) => s + (perZone[z]?.dailyUniques[i] || 0), 0));
            ...
            const chartUniques = lastDates.map((_, i) => zonesOfInterest.reduce((s, z) => s + (perZone[z]?.dailyUniques[i] || 0), 0));
        ...
        function renderRuralZoneSection() {
            ...
            function buildRuralTableHtml() {
                ...
                const grandUniques = lastDates.map((_, i) => top20Zones.reduce((s, z) => s + (perZone[z]?.dailyUniques[i] || 0), 0));
            ...
            const chartUniques = lastDates.map((_, i) => top20Zones.reduce((s, z) => s + (perZone[z]?.dailyUniques[i] || 0), 0));
        ...
        function start() {
            ...
            loadClientBase()
                .then(() => {
                    if (statusText) statusText.textContent = 'Loading call data from Firebase...';
                    onValue(callsRef, (snapshot) => {
                        ...
                        if (statusText) statusText.textContent = 'Error loading data from Firebase. Check connection/rules.';
