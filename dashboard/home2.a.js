// home-2.js — FULL UPDATED VERSION with latest data date selection + all features working
...
function processRawData(data) {
  ...
  // FIXED: Select the latest date from actual data
...
function renderCombinedHourlyChart() {
  ...
  const options = {
    series: [
      ...
      { name: selectedDate, data: todayData.map(v => v ?? null) }
    ],
    ...
    xaxis: { type: 'category', categories: Array.from({ length: 24 }, (_, i) => `${i}:00`), labels: { style: { fontSize: '12px' } } },
    yaxis: { title: { text: 'Call Volume' }, labels: { formatter: val => Math.round(val) } },
...
function renderAgentVsSystemHourlyChart(agent) {
  ...
  const options = {
    series: [
      { name: agent, data: agentData.map(v => v ?? null) },
      { name: 'Team Total', data: systemData.map(v => v ?? null) }
    ],
    chart: { type: 'area', height: 380, toolbar: { show: false }, animations: { enabled: true } },
    ...
    tooltip: {
      ...
      intersect: false,
      x: { format: 'HH:mm' },
      y: { formatter: val => val ? val.toString() : '0' }
    },
    xaxis: {
      type: 'category',
    ...
    yaxis: { title: { text: 'Call Volume', style: { color: '#64748b' } } },
...
function renderTalkTimeComparisonChart(agent) {
  ...
  const options = {
    series: [
      ...
      { name: 'Team Total', data: systemData.map(v => v ?? 0) }
    ],
    chart: { type: 'area', height: 380, toolbar: { show: false }, animations: { enabled: true } },
    stroke: { curve: 'smooth', width: [4, 3] },
    fill: { type: 'gradient', gradient: { shadeIntensity: 1, opacityFrom: 0.6, opacityTo: 0.1, stops: [0, 90, 100] } },
    ...
    tooltip: {
      ...
      intersect: false,
      x: { format: 'HH:mm' },
      y: { formatter: val => val > 0 ? formatSecondsToMinutes(val) : '0:00' }
    },
    xaxis: {
      type: 'category',
      categories: Array.from({ length: 24 }, (_, i) => `${i}:00`),
    ...
    yaxis: { title: { text: 'Talk Time' }, labels: { formatter: val => formatSecondsToMinutes(val) } },
    legend: { position: 'top' },
    grid: { borderColor: '#e2e8f0', strokeDashArray: 4 }
  };
...
function renderAHTHeatmap(agent) {
  ...
  const options = {
    ...
    dataLabels: { enabled: false },
    tooltip: { y: { formatter: val => val === 0 ? 'No calls' : formatSecondsToMinutes(val) } },
    xaxis: { type: 'category', categories: Array.from({ length: 24 }, (_, i) => `${i}:00`) },
    yaxis: { title: { text: 'Average Handle Time' }, labels: { formatter: val => formatSecondsToMinutes(val) } },
    grid: { borderColor: '#e2e8f0', strokeDashArray: 4 }
  };
...
// RESET BUTTON — resets to latest data date
